...
def descargar_video(enlace):
    ...
    try:
        ...
        # Descargar el video en la ruta especificada y nombre aletoria
        import random
        lower="abcdefghijklmnñopqrstuwxyz"
        upper="ABCDEFGHIJKLMNÑOPQRSTUWXYZ"
        number="0123456789"
        simbol="!#$%>&¡?=)(/&@[]{-,}"
        all=lower+upper+number+simbol
        length=9
        nombre_de_la_musica="".join(random.sample(all,length))
        
        # Construir la ruta completa para guardar el video
        ruta_video = os.path.join(directorio_actual, nombre_de_la_musica+".mp4")

        descarga.download(output_path=directorio_actual, filename=nombre_de_la_musica+".mp4")
